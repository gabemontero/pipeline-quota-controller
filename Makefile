...
build:
	...
	env GOOS=linux GOARCH=amd64 go build -mod=vendor -o out/pipelinequtoacontroller ./cmd/controller
...
generate-crds:
	...
	"$(CONTROLLER_GEN)" "$(CRD_OPTIONS)" rbac:roleName=manager-role webhook paths=./pkg/apis/pipelinequotacontroller/v1alpha1 output:crd:artifacts:config=deploy/crds/base
...
dev-image:
	docker build . -t quay.io/$(QUAY_USERNAME)/stonesoup-pipelinerun-quota-controller:dev
	docker push quay.io/$(QUAY_USERNAME)/stonesoup-pipelinerun-quota-controller:dev
